...
def generar_pdf_reporte_cuadre(request, cuadre_id):
    # Obtener el cuadre y detalles
    cuadre = get_object_or_404(Cuadre, id=cuadre_id)
    detalles = cuadre.detalles.select_related('producto').order_by('producto__nombre')

    # Calcular totales (versión optimizada)
    total_efectivo = detalles.aggregate(total=Sum('importe_efectivo'))['total'] or 0
    total_transferencia = detalles.aggregate(total=Sum('importe_transferencia'))['total'] or 0
    total_gasto = detalles.aggregate(total=Sum('importe_gasto'))['total'] or 0
    total_salidas = detalles.aggregate(total=Sum('salidas'))['total'] or 0
    total_general = total_efectivo + total_transferencia
    ...
    # Usar nombre de archivo más simple
    filename = f"reporte_cuadre_{cuadre.fecha.strftime('%Y-%m-%d')}.pdf"
